...
public class DesUtility {
	...
	byte[] salt = { (byte) 0xA9, (byte) 0x9B, (byte) 0xC8, (byte) 0x32,
			(byte) 0x56, (byte) 0x35, (byte) 0xE3, (byte) 0x32 };
	...
	public DesUtility(String passPhrase) {

		try {
		 
			KeySpec keySpec = new PBEKeySpec(passPhrase.toCharArray(), salt,iterationCount);
			SecretKey key = SecretKeyFactory.getInstance("PBEWithMD5AndDES")
					.generateSecret(keySpec); // PBEWithMD5AndDES
	...
	private String numeriC(String inS) {
		...
		for (int i = 0; i < inS.length(); i++) {
			outS += Utils
					.getNumero((inS.substring(i, i + 1)).hashCode(), "000");
		}
